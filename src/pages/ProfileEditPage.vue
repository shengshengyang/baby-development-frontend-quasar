  ...
  <q-page class="q-pa-md">
    <q-card class="custom-card">
      ...
      <div class="action-buttons">
        ...
        <!-- 當非編輯狀態下才顯示編輯 icon -->
        <q-btn v-if="!editMode" flat round icon="edit" color="primary" @click="toggleEditMode" />
      </div>
      ...
      <q-card-section class="text-center">
        ...
        <div v-if="editMode">
          <q-input filled v-model="editedUsername" label="姓名" class="q-mb-sm" />
          <q-input filled v-model="editedEmail" label="電子信箱" class="q-mb-sm" />
        </div>
        <div v-else>
          ...
          <q-list bordered>
            <q-item v-for="(baby, index) in userData?.babies || []" :key="index">
              <q-item-section avatar>
                <q-radio
                  v-model="selectedBabyIndex"
                  :val="index"
                  @update:model-value="onBabySelected"
                />
              </q-item-section>
              <q-item-section>
                <div class="text-h6">{{ baby.name }}</div>
                <div class="text-caption">出生日期：{{ baby.birthDate }}</div>
                <div class="text-caption" v-if="index === selectedBabyIndex">
                  <q-badge color="primary">預設寶寶</q-badge>
                </div>
              </q-item-section>
            </q-item>
          </q-list>
          <div v-if="(userData?.babies || []).length === 0" class="text-center q-pa-md">
            <div class="text-subtitle2">尚未新增任何寶寶</div>
            <q-btn
              flat
              color="primary"
              icon="person_add"
...
const selectedBabyIndex = ref(userStore.selectedBabyIndex);
...
// 監聽 store 中 selectedBabyIndex 的變化
watch(
  () => userStore.selectedBabyIndex,
  (newVal) => {
    selectedBabyIndex.value = newVal;
  },
);

onMounted(() => {
  // 確保從 localStorage 載入已選擇的寶寶
  userStore.initSelectedBaby();
  selectedBabyIndex.value = userStore.selectedBabyIndex;
});

const onBabySelected = () => {
  userStore.selectBaby(selectedBabyIndex.value);
};
